...
static int vfio_register_display_notifier(VFIOPCIDevice *vdev)
{
    ...
    ret = event_notifier_init(&dpy->pri_event_notifier, 0);
    if (ret) {
        error_report("vfio: Unable to init event notifier for device request");
        goto out;
    }

    ret = event_notifier_init(&dpy->cur_event_notifier, 0);
    if (ret) {
        error_report("vfio: Unable to init event notifier for device request");
        goto out;
    }
    ...
    pfd[0] = event_notifier_get_fd(&dpy->pri_event_notifier);
    pfd[1] = event_notifier_get_fd(&dpy->cur_event_notifier);
    qemu_set_fd_handler(pfd[0], primary_plane_update, NULL, vdev);
    qemu_set_fd_handler(pfd[1], cursor_plane_update, NULL, vdev);
    ret = ioctl(vdev->vbasedev.fd, VFIO_DEVICE_SET_IRQS, irq_set);
    if (ret) {
        error_report("vfio: Failed to set up device request notification");
        qemu_set_fd_handler(pfd[0], NULL, NULL, vdev);
        qemu_set_fd_handler(pfd[1], NULL, NULL, vdev);
        event_notifier_cleanup(&dpy->pri_event_notifier);
        event_notifier_cleanup(&dpy->cur_event_notifier);
    }
                                       ...
                                       uint32_t type, uint32_t subtype)
{
    ...
    g_free(irq_set);
    qemu_set_fd_handler(event_notifier_get_fd(&dpy->pri_event_notifier),
                        NULL, NULL, vdev);
    qemu_set_fd_handler(event_notifier_get_fd(&dpy->cur_event_notifier),
                        NULL, NULL, vdev);

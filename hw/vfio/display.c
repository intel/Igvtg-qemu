...
static void vfio_display_dmabuf_update(void *opaque)
{
    ...
    if (!dpy->event_flags || (dpy->event_flags & VFIO_CON_REFRESH_EVENT)) {
    ...
    if (!dpy->event_flags || (dpy->event_flags & (VFIO_CUR_REFRESH_EVENT |
				    VFIO_CON_REFRESH_EVENT))) {
...
int _event_notifier_test_and_clear(EventNotifier *e, guint64 *val_64)
{
    int value;
    ssize_t len;
    char buffer[512];
    ...
    if (value)
	    *val_64 = buffer[0] + ((guint64)(buffer[1]) << 8) +
		    ((guint64)(buffer[2]) << 16) +
		    ((guint64)(buffer[3]) << 24) +
		    ((guint64)(buffer[4]) << 32) +
		    ((guint64)(buffer[5]) << 40) +
...
static void vblank_handler(void *opaque)
{
    ...
    if (!_event_notifier_test_and_clear(&dpy->vblank_notifier, &val)) {
        vfio_unmask_single_irqindex(&vdev->vbasedev, dpy->irq_index);
        return;
    }

    if (val >= VFIO_CON_REFRESH_EVENT_INC) {
	dpy->event_flags |= VFIO_CON_REFRESH_EVENT;
	...
	counter++;
    } else if (val >= VFIO_CUR_REFRESH_EVENT_INC)
        dpy->event_flags |= VFIO_CUR_REFRESH_EVENT;
